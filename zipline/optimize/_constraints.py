...
class BaseConstraint(object):
    ...
    def constraints(self, w, w_long=None, w_short=None, assets=None):
        """返回权重变量限制
        ...
        Args:
          ...
          assets：资产索引
        """
        return self._constraints_list(w, w_long, w_short, assets)

    @abstractmethod
    def _constraints_list(self, w, w_long, w_short, assets):
        """子类完成具体表达式"""
        pass
...
class NetExposure(BaseConstraint):
    ...
    def _constraints_list(self, w, w_long, w_short, assets=None):
        if w_long is None:
            raise ValueError('w_long不得为空')
        if w_short is None:
            raise ValueError('w_short不得为空')
        total = cvx.sum(w_long + w_short)
...
class DollarNeutral(BaseConstraint):
    ...
    def _constraints_list(self, w, w_long, w_short, assets=None):
        diff = cvx.sum(w_long) + cvx.sum(w_short)
        return [
...
class NetGroupExposure(BaseConstraint):
    ...
    def __init__(self, labels, min_weights, max_weights, etf_lookthru=None):
        ...
        assert min_weights.index.equals(max_weights.index), '最大最小权重限制其索引应一致'
        self.labels = labels
    ...
    #             g = self.labels[idx]
    #             group_l_w[g].append(w_long[i])
    #             group_s_w[g].append(w_short[i])
...
# def _gen_asset_constraints(indexes, min_, max_, long_vars, short_vars):
#     """
#     生成资产权重限定表达式列表(内部辅助函数)
...
# def _gen_group_constraints(groups, indexes, min_series, max_series, long_vars,
#                            short_vars):
#     """
#     生成分组限定表达式列表(内部辅助函数)
...
#             # 最小值、最大值均位于零点右侧
#             constraints.extend(
#                 [w_short[i] >= 0, w_long[i] >= min_, w_long[i] <= max_])
#         else:
#             if max_ >= 0:
#                 # 最大值位于零点右侧，而最小值位于左侧
#                 constraints.extend([
#                     w_long[i] >= 0, w_long[i] <= max_, w_short[i] <= 0,
#                     w_short[i] >= min_
#                 ])
#             else:
#                 # 最小值、最大值均位于零点左侧
#                 constraints.extend(
#                     [w_long[i] <= 0, w_short[i] >= min_, w_short[i] <= max_])
#     return constraints
